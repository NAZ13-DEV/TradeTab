document.addEventListener('DOMContentLoaded', function () {
  ...
  axios
    .get('../newApi/api/task/FetchAllprofitLoss')
    .then(function (response) {
      ...
      if (response.status === 201) {
        ...
        axios
          .get(
            `../newApi/api/task/FetchAllprofitLossLimit/${currentPage},${itemsPerPage}`,
          )
          .then(function (response) {
            if (response.status === 201) {
...
<td>
  <button data-user-id="${id}" class="btn btn-warning btn-md edit-deposit-details">Edit profit/loss details</button>
</td>
<td>
...
`;
  ...
  tablebody.addEventListener('click', function (event) {
    if (event.target.classList.contains('disapprove-deposit')) {
      ...
      axios
        .patch(`../newApi/api/task/disapproveprofitLoss/${userid}`)
        .then((response) => {
          const result = response.data.message;
          if (response.status === 201) {
  ...
  tablebody.addEventListener('click', function (event) {
    if (event.target.classList.contains('approve-deposit')) {
      const userid = event.target.dataset.userId;
      axios
        .patch(`../newApi/api/task/approveprofitLoss/${userid}`)
        .then((response) => {
          const result = response.data.message;
          if (response.status === 201) {
            const message = result;
  ...
  tablebody.addEventListener('click', function (event) {
    if (event.target.classList.contains('delete-deposit')) {
      ...
      axios
        .delete(`../newApi/api/task/deleteprofitLoss/${userid}`)
        .then((response) => {
          const result = response.data.message;
          if (response.status === 201) {
            const message = result;
  ...
  tablebody.addEventListener('click', function (event) {
    if (event.target.classList.contains('edit-deposit-details')) {
      ...
      axios
        .get(`../newApi/api/task/fetchprofitLossWithId/${productId}`)
        .then(function (response) {
          if (response.status === 201) {
...
<td><input type="text" class="form-control" readonly value="${first_name} ${last_name}"></td>
 ...
 const type = inputs[8].value.trim();
    ...
    axios
      .put(`../newApi/api/task/editprofitLoss`, jsonData)
      .then(function (response) {
