  ...
  axios
    ...
    .then(function (response) {
      ...
      if (response.status === 201) {
        ...
        axios
          .get(
            `../api/task/FetchSharesLimit/${currentPage},${itemsPerPage}`,
          )
          .then(function (response) {
            if (response.status === 201) {
...
<td>${dateOfTrans}</td>
<td>
  <button data-user-id="${id}" class="btn btn-warning btn-md edit-deposit-details">Edit shares details</button>
</td>
<td>
  <button data-user-id="${id}" class="btn btn-danger btn-md delete-deposit">Delete shares details</button>
</td>
<td>
  <button data-user-id="${id}" class="btn btn-secondary btn-md disapprove-deposit">Disapprove shares</button>
</td>
<td>
  <button data-user-id="${id}" class="btn btn-success btn-md approve-deposit">Approve shares</button>
</td>
...
`;
  ...
  tablebody.addEventListener('click', function (event) {
    if (event.target.classList.contains('disapprove-deposit')) {
      ...
      axios
        .patch(`../api/task/disapproveShare/${userid}`)
        .then((response) => {
          const result = response.data.message;
          if (response.status === 201) {
  ...
  tablebody.addEventListener('click', function (event) {
    if (event.target.classList.contains('approve-deposit')) {
      const userid = event.target.dataset.userId;
      axios
        .patch(`../api/task/approveShares/${userid}`)
        .then((response) => {
          const result = response.data.message;
          if (response.status === 201) {
            const message = result;
  ...
  function fetchData() {
    axios
      .get(`../api/task/FetchSharesLimit/${currentPage},${itemsPerPage}`)
      .then(function (response) {
        if (response.status === 201) {
          const result = response.data.message.deposits;
  ...
  tablebody.addEventListener('click', function (event) {
    if (event.target.classList.contains('edit-deposit-details')) {
      ...
      axios
        .get(`../api/task/fetchShareWithId/${productId}`)
        .then(function (response) {
          if (response.status === 201) {
...
<td><input type="text" class="form-control" readonly value="${first_name} ${last_name}"></td>
...
<td><input type="text" style="display:none" class="form-control" value="${id}"></td>
  ...
  function renderUpdatedRow(row, user, countContent, editedData) {
    ...
    axios
      .put(`../api/task/editShare`, jsonData)
      .then(function (response) {
