...
class UserGateway
{
    ...
    public function registerUser(array $data)
    {
        ...
        $columns = [
            ...
            'createdAt' => 'VARCHAR(200) NULL DEFAULT NULL',
            ...
            'signal_message' => 'VARCHAR(200) NULL DEFAULT NULL'
        ];
                            ...
                            if ($inserted) {
                                ...
                                if ($referrer !== 'null') {
                                    ...
                                    if ($data['referral'] !== 'null') {
                                        ...
                                        $fetchUserDetailsWithUsername = $this->gateway->fetchData(RegTable, $conditions);
                                        $name = $fetchUserDetailsWithUsername['firstName'] . ' ' . $fetchUserDetailsWithUsername['last_Name'];
                                        $date = date('d-m-Y h:i:s a', time());
                                        if ($fetchUserDetailsWithUsername) {
                                            $referralData = [$name, $fetchUserDetailsWithUsername['id'], $id, $referralID, $date, '0.00', 'Pending'];
                                            $createReferalTable = $this->createDbTables->createTable(referralTable, $referralColumns);
                                            if ($createReferalTable) {
                                                $referralBindingArray = $this->gateway->generateRandomStrings($referralColumns);
                                                $insertreferal = $this->conn->insertData($connecter, referralTable, $referralColumns, $referralBindingArray, $referralData);
                                                if ($insertreferal) {
                                                    $h = 'Successful Registration';
                                                    $c = 'Congratulations! Your registration was successful.';
                                                    $message = $this->gateway->createNotificationMessage($id, $h, $c);
                                                    ...
                                                    $errors[] = 'could not insert referal data';
                                                    if (!empty($errors)) {
                                                        $this->response->respondUnprocessableEntity($errors);
                                                        return;
                                                    }
                                                ...
                                                $errors[] = 'The table ' . referralTable . ' could not be created.';
                                                if (!empty($errors)) {
                                                    $this->response->respondUnprocessableEntity($errors);
                                                    return;
                                                }
    ...
    {
        $drivingLicenseBack = json_encode($this->gateway->processImageWithgivenNameFiles($file['drivingLicenseBack']));
        $drivingLicenseFront = json_encode($this->gateway->processImageWithgivenNameFiles($file['drivingLicenseFront']));
        $userId = $id['userId'];
        $date = date('Y-m-d H:i:s', time());
        // try {
        $walletColumnArray = ['userid', 'DocumentType', 'FrontView', 'BackView', 'Status', 'submitDate', 'approveDate'];
        $result = $this->createDbTables->createTable(kyc, $walletColumnArray);
        if ($result === true || $result === null) {
            $data = [$userId, 'Drivers License', $drivingLicenseFront, $drivingLicenseBack, 'pending', $date, null];
