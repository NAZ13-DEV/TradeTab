...
class DepositGateway
{
    ...
    {
        try {
            $depositColumnArray = ['amtValue', 'amount', 'transMethod', 'userid', 'Wallet', 'transId', 'userBalance', 'transStatus', 'dateOfTrans', 'noOfShares' ];
    ...
    {
        try {
            $depositColumnArray = ['userid','plan', 'picture','transId', 'dateOfTrans'];
    ...
    { 
        try {
            $depositColumnArray = ['cryptovalue', 'cryptoAmt', 'netWork', 'sessionGetUserID', 'companyWallet', 'selectedPlan', 'transId', 'balance', 'transStatus', 'dateOfTrans'];
            ...
            if ($result === true || $result === null) {
                ...
                if ($inserted) {
                    ...
                    $message = $this->gateway->createNotificationMessage($plandepositPagedata['sessionGetUserID'], $h, $c);
                    if ($message) {
                        // $sent = $this->mailsender->sendRegistrationEmail($userEmail, $userFullname, $userId, $EncrypteduserEmail, $encodedId);
                        // if ($sent === true) {
