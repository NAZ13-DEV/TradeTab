...
class Database
{
    ...
    // public function insertDataWithTypes( $pdo, string $tableName, array $columns, array $bindingArray, array $data)
    // {
    ...
    {
        // Ensure column keys match data keys (case-insensitive match)
        $normalizedColumns = [];
        foreach ($columns as $key => $type) {
            $normalizedColumns[strtolower($key)] = $type;
        }
        ...
        if (count($normalizedColumns) !== count($normalizedData)) {
            throw new InvalidArgumentException("Number of columns and data elements must match");
        }

        // Ensure the same keys in same order
        $columnNames = array_keys($normalizedColumns);
    ...
    //     foreach ($data as $key => $value) {
    //         if ($key === 'content' && strlen($value) > 1000) { // Replace 255 with the actual column size
    ...
    //     foreach ($bindingArray as $index => $bindKey) {
    //         $stmt->bindValue(':' . $bindKey, $data[$bindKey] ?? null);
    //     }
