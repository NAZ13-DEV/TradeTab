document.addEventListener('DOMContentLoaded', function () {
    ...
    axios.get('../api/task/ViewuploadProof')
        .then(function (response) {
            ...
            if (response.status === 201) {
                ...
                axios.get(`../api/task/FetchAllUProofLimit/${currentPage},${itemsPerPage}`)
                    .then(function (response) {
                        const result = response.data.message;
                        if (response.status === 201) {
    ...
    function renderTableRows(data) {
        ...
        data.forEach((element, index) => {
            ...
            const { id, picture, userid, transId, dateOfTrans } = element;
            axios.get(`../api/task/fetchUserDetails/${userid}`).then((response) => {
              ...
              <tr>
                  ...
                  <td> <img src="${Img}" alt="proof Image" height='80' width='80'></td> 
                  <td>${transId}</td>
                  <td>${dateOfTrans}</td>
                  <td>
                  <button data-user-id="${id}" class="btn btn-danger btn-md delete-user">Delete proof of Payment</button>
                  </td>
              </tr>
            `;
    ...
    tablebody.addEventListener('click', function (event) {
        if (event.target.classList.contains('delete-user')) {
            ...
            axios.delete(`../api/task/deletupload/${userid}`)
                .then((response) => {
                    const result = response.data.message;
                    if (response.status === 201) {
