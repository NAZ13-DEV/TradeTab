document.addEventListener('DOMContentLoaded', function () {
  ...
  function countdown(Deposit) {
    ...
    timer = setInterval(() => {
      ...
      if (seconds === 0) {
        ...
        countdownElement.innerText = '00:00:00';
        axios
          .delete(`../newApi/api/task/deletShare/${Deposit}`)
  ...
  function checkStatus(Deposit, usdAmount, bitfr, symboll) {
    const fetchEmailStatusIntervalId = setInterval(function () {
      axios
        .get(`../newApi/api/task/checkSharesPage/${Deposit}`)
        .then((response) => {
          if (response.status === 201) {
        ...
        axios
          ...
          .then((response) => {
            ...
            getCryptoData(cryptoSymbol).then((data) => {
              ...
              axios
                .post('../newApi/api/task/sharePage', requestData)
                .then((response) => {
                  if (response.status === 201) {
                    const readResult = response.data.message;
                    ...
                    if (readResult.message === 'true') {
                      Toasty.showToast(
                        'success',
                        `your Share Purchase has been recorded you are to pay ${usdAmount} usd in value ${bitcoinFraction.toFixed(2)} ${symbol.toUpperCase()}`,
                      );
                    }
                    countdown(depositId);
