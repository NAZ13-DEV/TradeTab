document.addEventListener('DOMContentLoaded', function () {
  ...
  axios
    ...
    .then(function (response) {
      ...
      if (response.status === 201) {
        ...
        axios
          .get(
            `../newApi/api/task/FetchAlltradersssLimit/${currentPage},${itemsPerPage}`,
          )
          .then(function (response) {
            if (response.status === 201) {
              const result = response.data.message;
  ...
  function renderTableRows(data) {
    ...
    data.forEach((element) => {
      ...
      const { id, traderName, traderNameAbb, winRate } = element;
      html += `
<tr>
...
<td>${winRate}</td>
<td>
  <button data-user-id="${id}" class="btn btn-danger btn-md delete-deposit">Delete trader</button>
</td>
...
`;
  ...
  tablebody.addEventListener('click', function (event) {
    if (event.target.classList.contains('delete-deposit')) {
      ...
      axios
        .delete(`../newApi/api/task/deletetraders/${userid}`)
        .then((response) => {
          const result = response.data.message;
          if (response.status === 201) {
  ...
  form.addEventListener('submit', function (event) {
    ...
    // Collect data
    const data = {
      bitcoin: bitcoin.value.trim(),
      ethereum: ethereum.value.trim(),
      tether: tether.value.trim(),
    };

    // Send Axios request
    axios
      .post('../newApi/api/task/submitTrader', data)
      .then(function (response) {
        if (response.status === 201) {
